...
```
The `perf` monitor also records lock contention when the kernel exposes the required `perf lock` tracepoints. If those tracepoints are unavailable, CSB keeps the normal flamegraph output and skips only the lock-contention graph.
