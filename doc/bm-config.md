...
- `"scheduler_scalability"`:  Records a complete previous-CPU to wakeup-target-CPU to actual-run-CPU matrix, migrations, target misses, wakeup-to-run latency, and target-task preemptions with bpftrace. The optional first argument overrides the application `comm` filter.
## PlotType
