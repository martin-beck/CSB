...
class MonitorType(str, Enum):
    ...
    IPI_TLB: Records IPI source/target/callsite matrices, handler time, and TLB shootdown amplification.
    """
...
class BenchmarkConfig(dict):
    ...
    def __resolve_monitor_dependency(
        ...
        if MonitorType.IPI_TLB in monitors and not IpiTlb.is_supported():
            bm_log(
                f"{MonitorType.IPI_TLB.value} tracepoints are not supported. This monitor is auto-removed!",
                LogType.ERROR,
            )
            del monitors[MonitorType.IPI_TLB]
        ...
        dependent_monitors = {MonitorType.PERF_LOCK, MonitorType.IPI_TLB} & monitors.keys()
        ...
        if MonitorType.IPI_TLB in monitors:
            perf_args.extend(IpiTlb.get_args())
            perf_args.extend(monitors[MonitorType.IPI_TLB])

        resolved_monitors: dict[MonitorType, list[str]] = {}
        resolved_monitors[MonitorType.PERF] = perf_args
        for monitor_type in (MonitorType.PERF_LOCK, MonitorType.IPI_TLB):
            if monitor_type in monitors:
                resolved_monitors[monitor_type] = monitors[monitor_type]

        for k, v in monitors.items():
            if k not in {MonitorType.PERF, MonitorType.PERF_LOCK, MonitorType.IPI_TLB}:
