...
static uint32_t wait_event_for_step(size_t step) {
  return eops[step].is_write ? EPOLLIN : EPOLLOUT;
}

static void config_wait(struct conn_data *d, struct epoll_st *est) {
  size_t step = d->step;
...
static bool resync_step(struct conn_data *d) {
...
static void track_conn(struct conn_data *d, struct epoll_st *est) {
  d->next = est->conns;
  est->conns = d;
  est->nconn++;
}

static void untrack_conn(struct conn_data *d, struct epoll_st *est) {
...
static void unregister(struct conn_data *d, struct epoll_st *est) {
...
static void handle_stalled_conn(struct conn_data *d, struct epoll_st *est) {
  d->stalled_waits++;
  // An idle poll is not proof of divergence; allow a bounded grace period.
  if (d->stalled_waits <= MAX_STALLED_WAITS) {
    return;
  }
  if (!resync_step(d)) {
    unregister(d, est);
...
static void handle_stalled_conns(struct epoll_st *est) {
...
static bool register_new(struct epoll_st *est, struct sockaddr *serv_addr,
                         size_t addr_size, bool retry_on_fail) {
...
static void readwrite(struct epoll_event *ev, struct epoll_st *est) {
...
int main(int argc, char *argv[]) {
  ...
  while (est.nconn) {
    ...
    int nfds = epoll_wait(est.fd, &evs[0], MAX_EVS, NETOPS_STALL_TIMEOUT_MS);
    if (nfds == -1) {
      continue;
    }
    if (nfds == 0) {
