...
const static char* UNIQUE_VAR(filenames)[1] = {"./tmp/rocksdb_db/MANIFEST-000005"};
...
memcpy((void*)(0x20900540ul+PTR_OFFSET), "./tmp/rocksdb_db/MANIFEST-000005\000", 33);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x20900540ul+PTR_OFFSET, /*flags=O_TRUNC|O_NONBLOCK|O_CREAT|O_CLOEXEC|O_WRONLY*/0x80a41, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
...
//  ]
	res = syscall(__NR_write, /*fd=*/UNIQUE_VAR(ctx->r)[0], UNIQUE_VAR(ctx->writeBuffer), /*count=*/0x77ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  write arguments: [
//    fd: fd (resource)
//    buf: ptr[in, buffer] {
...
//  ]
	res = syscall(__NR_write, /*fd=*/UNIQUE_VAR(ctx->r)[0], UNIQUE_VAR(ctx->writeBuffer), /*count=*/0x11ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fdatasync arguments: [
//    fd: fd (resource)
//  ]
...
//  ]
	res = syscall(__NR_write, /*fd=*/UNIQUE_VAR(ctx->r)[0], UNIQUE_VAR(ctx->writeBuffer), /*count=*/0x79ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  write arguments: [
//    fd: fd (resource)
//    buf: ptr[in, buffer] {
