...
memcpy((void*)(0x202032c0ul+PTR_OFFSET), "./tmp/rocksdb_db\000", 17);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x202032c0ul+PTR_OFFSET, /*flags=O_NONBLOCK|O_DIRECTORY*/0x4800, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fsync arguments: [
//    fd: fd (resource)
//  ]
